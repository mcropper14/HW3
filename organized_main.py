...
def train_model(model, train_loader, criterion, optimizer, scheduler, num_epochs=args.epochs, patience=args.patience, checkpoint_dir="checkpoints"):
    ...
    for epoch in range(num_epochs):
        ...
        val_losses.append(evaluate_model(model, test_loader, criterion))

        logger.info(f'Epoch [{epoch+1}/{num_epochs}], Loss: {avg_loss:.4f}')
        torch.save(model.state_dict(), os.path.join(checkpoint_dir, f'{args.model}_{args.dataset}_epoch_{epoch+1}_loss_{avg_loss:.4f}.pth'))
